...
5. **The check is required; the script is not.** A live-check script (e.g. `*.live-check.ts`) is throwaway: write it, run it against reality, paste its actual output into the PR as the acceptance evidence, then discard it — do not commit it, and do not redirect it into `.claude/loop-evals/` (out-of-tree evals are invisible to reviewer agents, which then falsely block on "no acceptance evidence"). If a check earns its keep as permanent coverage, promote it into `test/integration/`, where CI actually runs it, instead of leaving it as a loose script.
